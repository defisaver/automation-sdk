...
function parseLlamaLendLeverageManagement(position: Position.Automated, parseData: ParseData): Position.Automated {
  const _position = cloneDeep(position);

  const { subStruct, subId, subHash } = parseData.subscriptionEventData;
  const { isEnabled } = parseData.strategiesSubsData;
  const triggerData = triggerService.llamaLendRatioTrigger.decode(subStruct.triggerData);
  const subData = subDataService.llamaLendLeverageManagementSubData.decode(subStruct.subData);
...
const parsingMethodsMapping: StrategiesToProtocolVersionMapping = {
  ...
  [ProtocolIdentifiers.StrategiesAutomation.LlamaLend]: {
    [Strategies.Identifiers.Repay]: parseLlamaLendLeverageManagement,
    [Strategies.Identifiers.Boost]: parseLlamaLendLeverageManagement,
  },
